//SNAP91_Single imageMIP better run
...
void filterHessian(string imname, Mat image, ofstream &myfile)
{
	...
	string name = format("%s_dend_processed.png", imname.c_str(), imname.c_str());
	...
	myfile << imname << " ," << countofdendrites << " ," << developed << ", " << lessdeveloped << ",";
...
Mat findsoma(Mat im, Mat imr, vector<vector<Point> > blue_contours, vector<vector<Point> > intersection_contours, ofstream &myfile, string fstr)
{
	...
	else
	{
		...
		{
			drawContours(outputImage, final_soma_contours, k, Scalar(20, 230, 125,255), 2, cv::LINE_8, vector<Vec4i>(), 0, Point());
...
int main(int argc, char** argv)
{
	...
	// Read image
		...
		cout << format("Processing Folder %s", argv[1]) << endl;
		for (int n = 1; n <= 40; n++)//differnt z scales
		{
		TOT:
			if(n<10)
				name = format("%s//%s_z0%dc1+2+3+4.tif", argv[1], argv[1], n);
			else
				name = format("%s//%s_z%dc1+2+3+4.tif", argv[1], argv[1], n);
				cout<<name<<endl;
		
			//name = format("C://Users//VIneeta//Pictures//STEPi//%s//%s_z%dc1+2+3.jpg", .c_str(), .c_str(), n);
			//cout << name << endl;
			Mat imm = imread(name);
			if (imm.empty()) //if image is empty- read next line of txt file- move on to the next folder-we did not find a particular z layer (process with stack created till now)
			{
				...
				cout << "No more images" << endl;goto POP;
			}
			//string result = format("C://Users//VIneeta//Documents//Visual Studio 2013//Projects//OpenCV-Test//OpenCV-Test//Old STEP1i//%s//z%d_3layers_processed.tif", .c_str(), n);
			string result = format("%s//%s_processed.png", argv[1],argv[1]);
	...
	POP:
		...
		resultr = temp;
		//	imshow(" mip",result);
		string stringb = format("%s_blue_MIP.png", argv[1], argv[1]);
		string stringg = format("%s_green_MIP.png", argv[1], argv[1]);
		string stringr = format("%s_red_MIP.png", argv[1], argv[1]);
		imwrite(stringb, resultb);
		imwrite(stringg, resultg);
		imwrite(stringr, resultr);
		//Mat bluegreen = resultb + resultg;
		string imagnam = format("%s", argv[1]);
		...
		/*~~~~~~~*/ Mat imblue= findsoma(resultb, resultr, blue_contours, intersect_contours, myfile, argv[1]);
		string out_string = format("%s_blue_processed.png", argv[1],argv[1]);
		imwrite(out_string, imblue);
		myfile << endl;
